...
> 由外层 agent（Claude Code / Cursor / DSH 等）按决策树调用第一层 25 个工具、
...
4. **想用的方法未实现 → 如实告知"暂不支持"，禁止心算替代**（如非参检验：hypothesis_test 只给警示）
...
- 未采用的方法及原因：<如"数据非正态，建议 Wilcoxon 但本工具集未实现"（若有）>
...
- 不向 server 加代码 → 第一层 25 工具全保持纯计算（可测可追责）；
