...
3. **口径钉死并可复算**：q1/q3=linear 插值（Excel QUARTILE.INC 同口径）、偏度/峰度=scipy Fisher 口径、std=ddof=1（Excel STDEV.S）——**文档写明，测试对手算公式和标准库独立核对**（**389 个 pytest**，覆盖见 statlab_mcp/docs/；CI 自动核对本数字，漂移即红）
